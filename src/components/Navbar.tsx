...
const navOptions = [
  ...
  { to: '/wallet-tokens', label: 'My Tokens', end: false },
  { to: '/send-tokens', label: 'Send Tokens', end: false },
]
...
export default function Navbar() {
  ...
  return (
    <aside className="sticky top-0 bg-linear-to-b from-slate-900 via-slate-950 to-black flex h-screen w-56 shrink-0 flex-col border-r border-slate-800/70 bg-slate-900/40 backdrop-blur">
      <div className="flex flex-col gap-6 p-4">
        ...
        {wallet.connected && solBalance !== null && (
          <div className="mt-4 rounded-xl border border-slate-800/70 bg-slate-800/40 px-4 py-3">
            <p className="text-xs font-medium text-slate-400 uppercase tracking-wider">Balance</p>
            <p className="mt-1 text-lg font-semibold text-emerald-400">{solBalance.toFixed(4)} SOL</p>
          </div>
        )}
